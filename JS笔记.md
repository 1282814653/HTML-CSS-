...
**强调 ,涉及到隐式转换时 所使用的时 String() Number() Boolean()**
...
- 逻辑运算符
  ...
  - 一是 || 或 当或的两边的参数判断结果都是 false 的时候 返回的结果就是 false 其他情况 都是 true
...
1. 选择结构
- if(){} 条件判断语句
- if(){}else if(){} 条件分支语句 (也称为跳楼现象) 每一条语句上都是一种选择
...
- while()和 do{}while()循环 他们 两个也都是循环语句 使用的程度相对于 for 循环而言 使用率较低.while 循环先判断在执行 如果 判断不通过 不会执行性 或者是死循环. do{}while() 循环是先执行一遍不看条件 如果 条件没有达到执行的要求 也会先执行然后在判断
## JS的字符串方法
- var str = "how are you? and you?";
- str.charAt(5); //r  返回指定位置的字符是什么(从0开始)
- str.charCodeAt(5)//114 返回指定位置的字符的Unicode 编码
- str.indexOf('0')// 1 返回 o 在字符串中的位置
- str.lastindexOf('0') //18 返回 o 在字符串的位置(从后往前)
- str.concat(str2)// 拼接字符串 组成新的字符串
- str.slice(1,6)//'ow ar' 截取在数值范围的字符串
